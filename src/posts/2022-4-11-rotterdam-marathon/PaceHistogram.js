...
const generate_hover_text = (e, paces) => {
  return (
    'pace: ' + convert_fractional_pace_to_minute_and_seconds(e.x0) + ' - ' + convert_fractional_pace_to_minute_and_seconds(e.x1) +
...
const PaceHistogram = ({ athlete, data, title, category_filter }) => {
  ...
  var y_values = arr.map(e => { return e.length });
  ...
  var annotation = (
    'pace: ' + convert_fractional_pace_to_minute_and_seconds(get_athlete_pace(athlete)) + ' min/km<br>' +
    'Fastest ' + calculate_pace_percentage(get_athlete_pace(athlete), paces).toString() + '%'
  )
  var x_tick_vals = get_all_numbers_between(Math.floor(Math.min(...paces)), Math.ceil(Math.max(...paces)))
  var x_tick_text = x_tick_vals.map(e => { return convert_fractional_pace_to_minute_and_seconds(e) })
