...
description: "A tutorial in which we deploy cloud infrastructure using Terraform, and then use that infrastructure to 
deploy a Dockerized Flask API to Google Cloud Run."
author: "Florian Maas"
...
We fill in the command prompts, add flask to the environment and then start a shell session:
...
type = "info"
text = <span>
It's important that the name of the file is <code>app.py</code>, since that is what Flask will search for by default. If you really want to 
use a different file name, run <code>poetry add python-dotenv</code> and add a <code>.flaskenv</code> file 
with <code>FLASK_APP=file_name.py</code> as its contents.
</span>
></TextBox>
...
type = "info"
text = <span>
If you are using <code>pip</code> instead of <code>poetry</code>, remove the lines <code>POETRY_VERSION</code>&nbsp;
and <code>RUN pip install "poetry==$POETRY_VERSION"</code>. Then replace the other lines that mention <code>poetry</code>&nbsp;
 with <code>COPY ./requirements.txt /code/requirements.txt</code> and <code>RUN pip install -r requirements.txt</code> respectively.
</span>
></TextBox>
...
Docker will recognize that these files have not changed since the latest build, and use a cached image
...
type = "warn"
text = <span>
Only add the <code>noauth</code> policy to your API if you want to open it to the public. If your API uses or exposes
...
type = "warn"
text = <span>
Take caution when working with service accounts. You should keep your service account keys private and you should make sure they don't fall into the wrong hands.
If you are using Git for this project, don't forget to add the key to your <code>.gitignore</code> file. To read more about 
service account best practices, see&nbsp;
<a href = "https://cloud.google.com/iam/docs/best-practices-for-using-and-managing-service-accounts" target = "_blank">here</a>.
</span>
></TextBox>
