...
directory and initiate a Python environment. In this tutorial, we will use [Poetry](https://python-poetry.org/) to create the environment
and manage our dependencies. To create a new environment with Poetry, run:
...
For this project, we will use a very simple Flask API, that simply returns `"Hello, World!"`.
...
If we have built our image once, and later we make a change to our API, for example by adding a `.py` file or by modifying `app.py`, 
but we do not modify the environment, there is no need to install the environment again. 
Docker will recognize that these files have not changed, and use a cached image
...
In order to set those variables, we create a file called `variables.tf` and populate it as follows:
...
Where `project_id` is the name of my GCP project, and the region and zones are set to locations that are geographically close
to me. For a list of available regions and zones, see [here](https://cloud.google.com/compute/docs/regions-zones#available).
...
resource will expect to find an image called `my-api` withiun our created Artifact Registry repository. We also pass
...
> Note: Only add the `noauth` policy to your API if you want to open it to the public. If your API uses or exposes
> sensitive data, you should add some form of authentication to your API.
...
Earlier, we created a service account that ahs the right permissions to deploy our infrastrcture. Within the Google Cloud Platform,
...
three dots in the `Actions` column for your service account. Select `Manage Keys`, and then `Add Key > New Key`. Choose `JSON` as `Key Type`, and this 
should download a file to your local machine. Copy this file to the directory in which you have created your `main.tf` and `variables.tf` files, and rename it to
...
It is also possible to directly code the path to the service account into your `main.tf` file, but I prefer this approach
since it's easier to modify it when we want to deploy our infrastructure through CI/CD later.

> Note! You should keep your service account keys private and you should make sure they don't fall in the wrong hands.
> So, if you are using Git for this project, don't forget to add the key to your `.gitignore` file. To read more about 
> Service Account best practices, see [here](https://cloud.google.com/iam/docs/best-practices-for-using-and-managing-service-accounts).
...
## 6. Pushing the container to Artifact Registry on Google Cloud

In order ot push our Docker image to the cloud, we first need to download the key for the service account `docker-pusher` that we created 
...
cloud_run_instance_url = "https://api-test-qaz3iff6ea-ez.a.run.app"
```
...
I hope this post was helpful toy ou, please let me know if you have any feedback!
