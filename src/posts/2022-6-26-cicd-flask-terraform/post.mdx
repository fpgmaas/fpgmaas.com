...
CI/CD pipelines**. In this tutorial, we will aim to cover these topics.
...
<TextBox
  ...
  text=<span>
    ...
    we will assume we have not deployed the infrastructure or the API yet. If you have the infrastructure 
...
error that the Docker image required to deploy the front-end could not be found. This was expected, since we needed to ceate the
...
We can use this argument to only deploy a subset of our infrastructure when we deploy it for the first time.
If `first_time` is `true`, we want Terraform to deploy all infrastructure except the Cloud Run instance. If `first_time`
is set to `false`, we want Terraform to deploy all infrastructure *including* the Cloud Run instance. To do so, we can add a `count` argument 
