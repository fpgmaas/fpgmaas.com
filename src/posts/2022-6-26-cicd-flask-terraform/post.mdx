...
<TextBox
  ...
  text=<span>
    ...
    we will assume we have not deployed the infrastructure or API yet. If you have the infrastructure 
    from the previous tutorial deployed, run <code>terraform destroy</code> before starting with this tutorial.
  </span>
></TextBox>
...
However, since we might want to do this in multiple projects and to make our approach reproducible, we will also provision this
...
a new module. To do so, create a new directory called `backend` with the files `main.tf` and `variables.tf`:
...
## 4. Deploying the initial infrastructure

When we attempted to deploy the infrastructure for the first time with `terraform apply` in the previous tutorial, we got an 
error that the Docker image required to deploy the front-end could not be found. Although this is not a problem, it also does not feel very clean
to expect an error when running our code. Instead, we are going to add a variable to `main/variables.tf`, indicating if we are deploying our 
...
We can use this argument to only deploy a subset of our infratructure when we deploy it for the first time.
If `first_time` is `true`, we want Terraform to deploy all infrastructure except the Cloud Run instance. If `first_time`
is set to `false`, we want to deploy all infrastructure *including* the Cloud Run instance. To do so, we can add a `count` argument 
...
To do so, we replace out output statement with
...
Assuming we already have sourced our `.env` file, we can now run the following commands to deploy our initial infrastructure.
...
Next, we set checkout our code and set up poetry. For setting up poetry, we see that another file in our repository is required. 
Create a new file `.github/workflows/setup-poetry-env/action.yml` and simply copy-paste the contents from 
[this file on GitHub](.github/workflows/setup-poetry-env/action.yml). For the scope of this tutorial, it suffices to know that this sets up
your Poetry environment, so we will not elaborate on the contents of this file.
...
# 6. Deploying our infrastructure with GitHub Actions
...
All that's left for us to do now is to deploy the Terraform infrastructure *including* the Cloud Run instance through GitHub Actions. To do so,
...
showing the contents of that file on this page, you can find them [here](https://github.com/fpgmaas/cloudrun-example-infra/blob/main/.github/workflows/on-pull-request.yml).
